##function determining stability of a model fitted using glmmTMB{glmmTMB}
###written by Roger Mundry
##it drops levels of the grouping variables ('random effects') one at a time and returns the range of estimates
##obtained when fitting the model to each of the subsets and also for each additional model
glmmTMB.stab<-function(model.res, contr=NULL, ind.cases=F, para=F, data=NULL, use=NULL, n.cores=c("all-1", "all"), save.path=NULL, load.lib=T, lib.loc=.libPaths()){
  ...
  ifun<-function(x, model.res, to.do, data, contr, extract.ranef.from.glmmTB, n.ranef, xfam){
    ...
    sel.ii.res=try(update(model.res, data=sel.ii.data), silent=T)
    #browser()
